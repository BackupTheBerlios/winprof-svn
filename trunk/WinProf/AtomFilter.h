...
public:
	...
	virtual bool IsDependantOn(CString n) const	{return false;}
	virtual bool DoesContain(CString n) const {return (name == n);}
	virtual bool IsAtom(void) const {return true;}
	virtual CString GetExpr(void) const {return CString("@");} // this is not an expression, but can serve as a marker

	// make use of a filter
	virtual bool Satisfies(const INVOC_INFO& iv) const
	{
		if(func != 0 && (only_this_func != (func == iv.address))) return false;
		if(stat != -1 && !stats[stat]->Satisfies(iv, bound, oper))return false;
